...
struct memory_map
{
	...
	unsigned int uniforms[NUM_QPUS][4];	// 4 parameters per QPU
	// first address is the input value
	// for the program to add to
	// second is the address of the
	// input buffer
	// third is the size of the input buffer
	// fourth is the address of the
	// result buffer
...
main (int argc, char **argv)
{
	...
	unsigned vc_input = ptr + offsetof(struct memory_map, input);
	...
	for (int i = 0; i < NUM_INPUTS; ++i) {
		arm_map->input[i] = 9990.0f + 3 * i;
	}
	for (int i = 0; i < NUM_QPUS; i++) {
		arm_map->uniforms[i][0] = uniform_val;
		arm_map->uniforms[i][1] = vc_input;
		arm_map->uniforms[i][2] = NUM_INPUTS;
		arm_map->uniforms[i][3] = vc_results + i * sizeof (unsigned) * 16;
